...
//	    	new Thread(new BiCoinlWorker(key, bicoinRate, bicoinInterval)).start();
	    	int minerInterval = Integer.parseInt(properties.get("miner.interval").toString().trim());
	    	String url = properties.get("miner.target").toString().trim();
	    	new Thread(new MinerWorker(minerInterval, url)).start();
		} catch (IOException e) {
			e.printStackTrace();
			System.exit(-1);
		}
